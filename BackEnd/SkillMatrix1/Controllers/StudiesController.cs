...
namespace SkillMatrix1.Controllers
{
    ...
    public class StudiesController : Controller
    {
        ...
        [HttpPost("{employeeId}")]
        public ActionResult<InterestDto> CreateInterestForEmployee(int employeeId, StudyDto study)
        {
            if (!_employeeRepository.EmployeeExists(employeeId))
            {
                return NotFound();
            }
            var studyMap = _mapper.Map<Study>(study);
            _studyRepository.AddStudyForEmployee(employeeId, studyMap);
